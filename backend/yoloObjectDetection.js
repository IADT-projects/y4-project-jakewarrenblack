...
const darknetPath = "./data/";

console.log(darknetPath)
...
//will receive a frame from runVideoDetection, which in turn gets its frames from grabFrames (via webcam)
const classifyImg = async img => {
    ...
    // looping over each detection, e.g. person, its bounding boxes, labels
     layerOutputs.forEach(mat => {
        ...
        output.forEach(detection => {
            ...
            if (confidence > minConfidence) {
                ...
                // pull info out of yolo detection and apply to our img
                indices.forEach(i => {
                    const rect = boxes[i];
