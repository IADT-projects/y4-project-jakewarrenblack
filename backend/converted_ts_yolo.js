 ...
 */
const { cv, getCachedFile, runVideoDetection } = require('./converted_utils')
...
class dnnDarknetYOLORealTimeObjectDetection {
    ...
    async run() {
        if (!cv.xmodules || !cv.xmodules.dnn) {
            console.error(
                `exiting: opencv4nodejs (${cv.version.major}.${cv.version.minor}) compiled without dnn module`
            )
            return
        }

        const darknetPath = "../data/dnn/yolo-object-detection"
        const cfgFile = await getCachedFile(
            `${darknetPath}/yolov3-tiny.cfg`,
            "https://raw.githubusercontent.com/pjreddie/darknet/master/cfg/yolov3-tiny.cfg",
            { notice: "See https://pjreddie.com/darknet/yolo/" }
        )
        const weightsFile = await getCachedFile(
            `${darknetPath}/yolov3-tiny.weights`,
            "https://pjreddie.com/media/files/yolov3-tiny.weights",
            { notice: "See https://pjreddie.com/darknet/yolo/" }
        )
        const labelsFile = await getCachedFile(
