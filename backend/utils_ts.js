...
const fs = require('fs')
const { cv, Mat, Rect, Vec3 } = require('@u4/opencv4nodejs');
...
function getCachedFile(localName, url, opts){
    opts = {};
...
if (opts.notice)
    ...
    return new Promise(async (done, reject) => {
        ...
        data.pipe(writer);
        data.on('error', (e) => { console.log('reject', e); reject(e); });
        data.on('close', () => {
 ...
 */
function cv_imshow(winName, img) {
    if (HEADLESS) {
...
function cv_setWindowProperty(winName, prop_id, prop_value) {
    if (!HEADLESS) {
