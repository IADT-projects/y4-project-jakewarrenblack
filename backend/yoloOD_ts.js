...
const { Mat, Net, Point2, Rect, Size, Vec3, VideoCapture, VideoWriter } = require('@u4/opencv4nodejs');
const { cv, getCachedFile } = require('./utils_ts');
...
const args = mri(process.argv.slice(2), { default: { device: 'cpu' }, alias: { h: 'help' } });
...
async function main() {
    ...
    const classesFile = path.resolve(__dirname, "data\\coco.names");
    // classes = None
    const classes = fs.readFileSync(classesFile, 'utf8').split(/[\r\n]+/);

    // Give the configuration and weight files for the model and load the network using them.
    const modelConfiguration = path.resolve(__dirname, "data\\yolov3-tiny.cfg")
    const modelWeights = path.resolve("data\\yolov3-tiny.weights")
    ...
    const drawPred = (frame, classId, conf, left, top, right, bottom) => {
        // Draw a bounding box.
    ...
    while (cv.waitKey(1) < 0) {
        ...
        //const layersNames = getOutputsNames(net);
        ...
        const outs = net.forward(layerNames)
