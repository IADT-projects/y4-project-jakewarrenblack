...
const BBoxAnnotator = React.forwardRef(({ url, borderWidth = 2, inputMethod, labels, onChange }, ref) => {
    ...
    return (<div className={classes.bBoxAnnotator} style={{
        ...
        fontSize: 'small',
    }} key={i} onMouseOver={() => setEntries((prevEntries) => prevEntries.map((e) => (e.id === entry.id ? { ...e, showCloseButton: true } : e)))} onMouseLeave={() => setEntries((prevEntries) => prevEntries.map((e) => (e.id === entry.id ? { ...e, showCloseButton: false } : e)))}>
                        {entry.showCloseButton ? (<div style={{
        position: 'absolute',
        top: '-8px',
        right: '-8px',
        width: '16px',
